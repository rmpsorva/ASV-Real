#!/bin/bash
# ===============================================================
# 🌐 launch_web_system.sh - Lanza Ollama y el Backend Flask
# Ejecuta los servicios en segundo plano para que la interfaz web funcione.
# ===============================================================
...
FLASK_FILE="backend/asva_flask_backend.py" # RUTA CORREGIDA
...
install_flask_dependencies() {
    ...
    if ! python3 -c "import flask" 2>/dev/null; then
        pip3 install flask flask-cors requests >> "$LOG_FILE" 2>&1
...
start_flask() {
    ...
    # RUTA CORREGIDA: Verifica si el archivo Flask existe en la carpeta 'backend/'
    ...
    # Esperar a que Flask esté listo (3 segundos es suficiente)
    sleep 3
...
check_services() {
    ...
    # Verificar Ollama (usando el endpoint /api/tags que siempre existe)
    if curl -s "http://localhost:${OLLAMA_PORT}/api/tags" >/dev/null 2>&1; then
    ...
    # Verificar Flask (usando el endpoint /api/query que siempre existe en POST)
    # Una simple verificación GET al puerto es suficiente para saber si el puerto está abierto.
...
start_system() {
    log_info "=========================================================="
    log_info "🚀 INICIANDO SISTEMA WEB COGNITIVO ASV-A"
    log_info "=========================================================="
    
    echo "=== ASV-A LAUNCH $(date) ===" > "$LOG_FILE"
    
    # 1. Verificar dependencias
    check_dependencies
    
    # 2. Instalar librerías de Flask (necesario si no se hizo en full_asv_deployment.sh)
    install_flask_dependencies
    ...
    log_info ""
    log_info "🛑 Para detener: ./launch_web_system.sh stop"
    log_info "=========================================================="
}
...
show_help() {
    echo -e "${CYBER_BLUE}"
    echo "🚀 ASV-A WEB SYSTEM LAUNCHER"
    echo -e "${NC}"
    echo "Uso: $0 [comando]"
    echo ""
    echo "Comandos:"
