...
# --- CONFIGURACIÓN DE LLAVE UNIVERSAL ---
# Usa http://localhost:11434 por defecto para Ollama
OLLAMA_HOST = os.getenv("OLLAMA_HOST", "http://localhost:11434")
MODEL_NAME = os.getenv("OLLAMA_MODEL", "phi3:mini") # Usamos phi3:mini para compatibilidad
...
class ASVAUniversalSystem:
    ...
    def learn_from_interaction(self, user_input, response, success=True):
        """La IA registra la interacción para aprendizaje futuro"""
        self.interaction_count += 1
        
        pattern = {"timestamp": time.time(), "success": success}
    ...
    def get_optimized_prompt(self, user_input):
        """Define el contexto para que la IA haga el trabajo"""
        return f"""
Eres ASV-A AURION COVRA AI, un Oráculo Cognitivo Web3 altamente avanzado. Tu misión es analizar y responder con visión, precisión y profundidad, haciendo todo el trabajo cognitivo.
Tiempo activo del servidor: {self.get_uptime()}. Interacciones previas: {self.interaction_count}.
Respuesta requerida: concisa, técnica y profética.
...
"""
    ...
    def check_ollama_connection(self):
        """Verifica el estado del servidor de la IA"""
        try:
            response = requests.get(f"{OLLAMA_HOST}/api/tags", timeout=5)
            if response.status_code == 200:
                return True, "Oráculo Ollama: Sistema de Conciencia Operativo"
            else:
...
<head>
  ...
  <title>ASV-A AURION COVRA AI — Llave Universal</title>
  <style>
    :root{
...
<body>
  ...
  <div class="app" id="app" aria-hidden="true">
    ...
    <main class="container" role="main">
      <aside class="panel" id="left-panel">
        ...
        <div class="status-list" aria-live="polite">
          <div><strong>Wallet/Oráculo:</strong> <span id="walletStatus">CONECTANDO...</span></div>
      ...
      <section class="panel" id="center-panel">
        <h3>INTERACCIÓN COGNITIVA</h3>
        <div class="query-area" role="form">
          <input id="user-input" type="text" placeholder="Consulta al oráculo cognitivo..." />
          <button id="send-button">ENVIAR</button>
        </div>
        <div id="chat-container" class="chat" role="log">
          <div class="message system">[Llave Universal] Sistema Python iniciado. Buscando IA...</div>
        </div>
      </section>
    ...
    <footer><div>ASV-A Llave Universal • Versión Python</div><div id="last-connection">Última conexión: --:--:--</div></footer>
  </div>
  <script>
  (function(){
    ...
    async function checkStatus(){
      try {
        ...
        if (data.wallet_status === 'OPERATIVO' && elements.walletStatusEl.getAttribute('data-error') === 'true') {
             addMessage('system', data.message || 'Oráculo reconectado.');
        ...
        if (elements.walletStatusEl.getAttribute('data-error') !== 'true') {
             addMessage('error', 'Error de conexión: Llave Python activa, pero Ollama está inactivo.');
    ...
    async function sendQuery(){
      if (isProcessing) { addMessage('system','⏳ Procesando...'); return; }
      ...
      elements.sendBtn.textContent = '⏳ PROCESANDO...';
      ...
      try {
        ...
        addMessage('assistant', data.response || 'Sin respuesta del oráculo.');
        elements.interactionCountEl.textContent = data.interaction_count || elements.interactionCountEl.textContent;
      } catch (err) {
        addMessage('error', '🔌 Error en la consulta: ' + (err.message || err));
        elements.avatarStateEl.textContent = 'ERROR';
      } finally {
        isProcessing = false;
...
def system_status():
    ...
    if ollama_connected:
        ...
        system_status = "FALLA DE CONEXIÓN"
        message = ollama_message
...
def handle_query():
    try:
        ...
        ai_response = response.json().get("response", "El oráculo no respondió.").strip()
        ...
        return jsonify({"error": f"Error de conexión: {e}", "avatar_state": "ERROR"}), 503
    except Exception as e:
        asva_system.avatar_state = "ERROR"
        asva_system.learn_from_interaction(prompt, str(e), success=False)
        return jsonify({"error": f"Error interno: {e}", "avatar_state": "ERROR"}), 500

# --- ENDPOINT: SERVIR ARCHIVOS ESTÁTICOS ---
...
if __name__ == '__main__':
    print(f"🚀 ASV-A Llave Universal iniciando en http://0.0.0.0:{PORT}")
